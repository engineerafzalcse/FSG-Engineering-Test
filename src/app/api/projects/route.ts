// ============================================================
// Route Handler — Projects (list + create)
// ============================================================
// Used with Pattern 2 (Route Handlers + ORM) or Pattern 3 (BFF).
//
// Fullstack (ORM): Replace fakeProjects calls with your ORM
//   const projects = await db.query.projects.findMany({ ... })
//
// BFF (proxy): Replace with fetch to your external backend
//   const res = await fetch(`${BACKEND_URL}/projects?${searchParams}`, {
...
export async function GET(request: NextRequest) {
  ...
  const data = await fakeProjects.getProjects({
    page,
    limit,
    statuses,
    search,
...
export async function POST(request: NextRequest) {
  ...
  const data = await fakeProjects.createProject(body);
